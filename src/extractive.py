...
class ExtractiveSummarizer(pl.LightningModule):
    ...
    def setup(self, stage):
        if stage == "fit": 
            ...
            if self.checkpoint is not None:
                ...
                self.list_train_acc_epoch = self.checkpoint['train_histories']['acc']

                self.list_val_loss_epoch = self.checkpoint['val_histories']['loss']
                self.list_val_acc_epoch = self.checkpoint['val_histories']['acc']
                
                self.min_loss = min(self.list_val_loss_epoch)
                logger.info("\nEpoch: {}\nTrain loss: {}\nTrain accuracy: {}\nVal loss: {}\nVal accuracy: {}\n".format(
                    self.epoch, str(self.list_train_loss_epoch), str(self.list_train_acc_epoch),
                    str(self.list_val_loss_epoch), str(self.list_val_acc_epoch)
                    ))
            else:
    ...
    def training_step(self, batch, batch_idx):
        ...
        preds = torch.flatten(y_hat)
        b_preds = preds.detach().cpu().numpy()
        labels = torch.flatten(labels)
        b_labels = labels.detach().cpu().numpy()
        self.train_preds.append(b_preds)
        self.train_labels.append(b_labels)
    ...
    def training_epoch_end(self, outputs): 
        ...
        threshold = 0.5
        self.train_preds = [preds > threshold for item in self.train_preds for preds in item]
        self.train_labels = [labels == 1 for item in self.train_labels for labels in item]
        train_accuracy = accuracy_score(self.train_labels, self.train_preds)

        self.list_train_loss_epoch.append(avg_train_loss)
        self.list_train_acc_epoch.append(train_accuracy)
        ...
        type_used_data = self.hparams.data_path.split('/')[-2].split('_')[-1]    
        if self.hparams.no_use_token_type_ids:
            temp_token_type_ids = "no-token-type-ids"
        else:
            temp_token_type_ids = "use-token-type-ids"
        temp_pooling_mode = self.hparams.pooling_mode.replace('_','-')
        model_name_or_path = self.hparams.model_name_or_path.replace('/','-')
        self.save_file = "{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}".format(
            model_name_or_path, type_used_data, temp_token_type_ids, temp_pooling_mode, self.hparams.classifier,
        ...
        train_histories = {
            ...
            'acc':self.list_train_acc_epoch
            }
        val_histories = {
            'loss':self.list_val_loss_epoch,
            'acc':self.list_val_acc_epoch
            }
    ...
    def validation_step(self, batch, batch_idx):
        ...
        labels = torch.flatten(labels)
        b_labels = labels.detach().cpu().numpy()
        self.val_preds.append(b_preds)
        self.val_labels.append(b_labels)
    ...
    def validation_epoch_end(self, outputs):
        ...
        threshold = 0.5
        self.val_preds = [preds > threshold for item in self.val_preds for preds in item]
        self.val_labels = [labels == 1 for item in self.val_labels for labels in item]
        ...
        self.list_val_acc_epoch.append(val_accuracy)
    ...
    def test_step(self, batch, batch_idx):
        ...
        labels = torch.flatten(labels)
        b_labels = labels.detach().cpu().numpy()
        self.test_preds.append(b_preds)
        self.test_labels.append(b_labels)
        return None
    ...
    def test_epoch_end(self, outputs):       
        ...
        threshold = 0.5
        self.test_preds = [preds > threshold for item in self.test_preds for preds in item]
        self.test_labels = [labels == 1 for item in self.test_labels for labels in item]
        test_accuracy = accuracy_score(self.test_labels, self.test_preds)
        ...
        logger.info("Report:\n%s", str(classification_report(self.test_labels, self.test_preds)))
        
        # Generate confusion matrix
        # cf_matrix = confusion_matrix(self.test_labels, self.test_preds)
